...
export function StickyCTA() {
  const { ranking, favoriteCount, hasFavorited } = useVotes();
    ...
    <div className="safe-bottom pointer-events-none fixed inset-x-0 bottom-0 z-40 px-4 pb-3">
      ...
      >
        {hasFavorited ? (
              ...
              {leader && (
                <span className="font-numeric truncate text-sm font-black">
                  ...
                  <span className="ml-1 font-sans text-[11px] font-bold text-amber-200">
                    {favoriteCount(leaderId).toLocaleString()}票
                  </span>
                </span>
              )}
            </span>
          ...
          <>
            <Heart className="h-5 w-5 shrink-0 text-amber-300" strokeWidth={2.25} fill="currentColor" />
            <span className="flex min-w-0 flex-1 flex-col items-start leading-tight">
              <span className="text-[10px] font-medium text-red-100">1人1票</span>
              <span className="text-sm font-black">お気に入りに投票しよう</span>
            </span>
          </>
        )}
